...
export function SettingsDialog() {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 backdrop-blur-[2px]">
      <div className="flex max-h-[85vh] w-[680px] max-w-[90vw] flex-col overflow-hidden rounded-xl border border-line bg-surface-elevated p-5 shadow-2xl">
        ...
        <div className="flex min-h-0 flex-1">
          <nav className="flex w-20 shrink-0 flex-col gap-0.5 border-r border-line pr-3">
            {SETTINGS_TABS.map((item) => (
              <button
                ...
                className={`rounded px-2 py-1.5 text-left text-[12px] ${
                  tab === item.id ? "bg-active text-ink" : "text-ink-subtle hover:bg-hover hover:text-ink"
                }`}
              >
                {item.label}
              </button>
          ...
          <div className="min-h-[280px] min-w-0 flex-1 overflow-auto pl-4">
            <div className="space-y-5">
              {tab === "appearance" && (
                <>
                  <section>
                    <div className="mb-2 text-[11px] font-medium uppercase tracking-wider text-ink-subtle">
                      主题
                    </div>
                    ...
                    <div className="flex gap-1.5">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          setBookmarks([
                            ...bookmarks,
                            { name: bookmarkName.trim(), path: bookmarkPath.trim() },
                          ]);
                          setBookmarkName("");
                          setBookmarkPath("");
                        }}
              ...
              {tab === "about" && (
                <>
                  <section>
                    ...
                    <p className="mt-1.5 text-[11px] text-ink-subtle">
                      按项目查看当前终端进程树的内存与 CPU，不含应用自身。
                    </p>
                  </section>

                  <section className="select-text text-[11px] leading-5 text-ink-subtle">
                    <div className="mb-2 text-[11px] font-medium uppercase tracking-wider text-ink-subtle">
                      版本
                    </div>
                    <div>
